...
export function createTestProduct(overrides: Partial<IProduct> = {}): Partial<IProduct> {
  return {
    ...
    price: 1000,
    currency: 'INR', // Will be set from DB in pre-save hook if not provided
    category: 'rings', // Must match an active category in DB
    material: 'Gold',
...
export function createTestOrder(overrides: Partial<IOrder> = {}): Partial<IOrder> {
  return {
    ...
    shippingAddress: {
      ...
      zipCode: '123456', // 6 digits for India (5 digits also allowed in test env)
      country: 'India',
 ...
 * NOTE: This is for local test database only, not real DB.
 * Real DB calls are mocked via database-mocks.ts
 */
...
/**
 * Get test address data with proper format
 * Uses mocked country settings (not real DB)
 */
export function getTestAddressData() {
  return {
    ...
    city: 'Test City',
    state: 'Test State', // Allowed in test environment
    zipCode: '123456', // 6 digits for India (5 digits also allowed in test)
    country: 'India',
    phone: '9876543210', // 10 digits for India
    countryCode: '+91',
  };
